...
const keyInstructions = {
  framing: [
    ...
    "- Why this framing (vs alternative): <1-2 bullets>",
  ...
  options: [
    "Output format:",
    "Options must differ by core mechanic or approach. If they are similar, merge them and create a real alternative.",
...
function buildKeyPrompt({ key, task, context, language, caseFile }) {
  ...
  return [
    ...
    "Always include a short 'Decision trail' section: Inputs → Options → Choice → Trade-offs.",
    "Decision trail must be 4–8 bullets max. It must show what you rejected and why.",
    "Do NOT repeat the same idea. Each section must add new information.",
